...
parser.add_argument("-m", "--meta", type=str, default="meta", help="The name of the meta file. default='videos.txt'")
...
parser.add_argument("-f", "--file", type=int, default=None, help="The id of the file")
...
if args.file:
    files = [files[args.file]]
speed = 2
img = None

if args.file:
    f = open(args.save+'/'+args.meta+"-"+str(args.file)+".txt", "w")
else:
...
try:
    for file in files:
        ...
        while cap.isOpened():
            ...
            while not k in [27]+[ord(x) for x in 'qQjlJLk,.yu']:
                if k == ord('s'):
                    lastS = pos
                elif k == ord('d'):
